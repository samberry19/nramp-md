...
Calculate the root mean square deviation (RMSD) of each frame in a molecular dynamics
...
rmsd = md.rmsd(traj, traj, frame=args.frame, atom_indices = traj.topology.select(sel))

if args.outpath == 'Auto':
    outpath = args.sim.split('.')[0] + '_rsmd.txt'
else:
